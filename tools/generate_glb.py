...
def make_role(role, spec):
    ...
    except Exception as exc: print('GLTF addon enable warning:', exc)
    ...
    action=bpy.data.actions.new('idle'); root.animation_data_create(); root.animation_data.action=action
    ...
    output=os.path.join(OUT,role+'.glb'); print('EXPORTING',role,'to',output)
