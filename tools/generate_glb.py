...
try:
    ...
    print('GLTF addon enable warning:', exc)
...
def cube(name, parent, loc, scale, material):
    ...
    o = bpy.context.object; o.name = name; o.scale = scale; o.data.materials.append(material); o.parent = parent
    return o
...
def make_role(role, spec):
    ...
    cube('Body', root, (0, 0, 1.0), (.42, .28, .65), body)
    sphere('Head', root, (0, 0, 2.0), (.32, .32, .32), body)
    if role == 'zhaoyun':
        cube('Sash', root, (0, 0, 1.15), (.5, .32, .08), accent)
        cube('Spear', root, (.62, 0, 1.2), (.06, .06, 1.2), accent)
    elif role == 'adou':
        cube('Ornament', root, (0, 0, 2.35), (.12, .12, .16), accent)
    else:
        cube('Shoulder', root, (0, 0, 1.52), (.52, .34, .12), accent)
    ...
    for clip in spec['clips'][1:]:
        ...
        track = root.animation_data.nla_tracks.new(); track.name = clip
        strip = track.strips.new(clip, 1, act); strip.action_frame_start = 1; strip.action_frame_end = 24
        strip.frame_start = 1; strip.frame_end = 24
    bpy.ops.object.select_all(action='SELECT')
    bpy.context.view_layer.objects.active = root
    output = os.path.join(OUT, role + '.glb')
    print('EXPORTING', role, 'to', output)
    result = bpy.ops.export_scene.gltf(filepath=output, export_format='GLB', use_selection=True, export_animations=True)
    print('EXPORT RESULT', role, result, os.path.getsize(output) if os.path.exists(output) else 'MISSING')
